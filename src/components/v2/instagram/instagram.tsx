...
export default async function InstagramPosts() {
    ...
    if(process.env.INSTAGRAM_KEY) {
        instagramFetch = await fetch(`https://graph.instagram.com/me/media?fields=id,caption,media_url,timestamp,media_type,permalink&access_token=${process.env.INSTAGRAM_KEY}`, { cache: 'no-store' }).then(res => res.json())
        
        
    
        console.log(instagramFetch)
        images = []
        if(instagramFetch.data) {
